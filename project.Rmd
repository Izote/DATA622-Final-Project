...
recipes_dbl <- recipes_dbl %>%
  filter(
      ...
      between(RecipeServings, boundary$RecipeServings[[1]], boundary$RecipeServings[[2]])
    )
```
...
recipe_pca <- recipes_dbl %>%
  ...
  mutate(
    across(c("Calories", "CarbohydrateContent"), sqrt),
    across(c("FatContent", "SodiumContent", "SugarContent"), ~ .^(1/3))
    ) %>% 
...
try_svm <- function(
  ...
  y <- if (type == "eps-regression") "AggregatedRating" else "Success"
  tune.svm(
    as.formula(paste0(y, " ~ PC1 + PC2 + PC3 + PC4 + PC5 + PC6")),
    data = data,
    type = type,
...
eval_regr <- function(data, model, new = FALSE) {
  ...
  list(
    MSE = Metrics::mse(data$AggregatedRating, prediction),
    RMSE = Metrics::rmse(data$AggregatedRating, prediction),
    MAE = Metrics::mae(data$AggregatedRating, prediction)
  )
}

eval_class <- function(data, model, new = FALSE) {
  if (new) {
...
full_pca_fct <- full_pca_data %>%
mutate(across(
  "AggregatedRating",
  ~ factor(if_else(. >= goal, 1, 0)), levels = c(0, 1))) %>% 
  rename("Success" = "AggregatedRating")
...
train2 <- map_dfr(model_data_fct, slice_sample, prop = 0.80)
test2 <- anti_join(bind_rows(model_data_fct), train2, by = "RecipeId")
```
...
best_class <- svm_class$best.model
```
...
```{r}
(class_eval <- eval_class(train2, best_class))
```
